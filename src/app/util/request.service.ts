...
export class RequestService {
  ...
  constructor(private http: Http, private router: Router, private authenticationService: AuthenticationService, private alertService: AlertService) {
  }
  ...
  private handleRequest(url: string, method: RequestMethod, body?: any, isVoid?: boolean, headers?: Headers): Promise<any> {
    var requestHeaders: Headers = headers == undefined ? this.headers : headers;
    ...
    return this.http.request(url, { headers: requestHeaders, method: method, body: body })
      ...
      .catch((error: any) => {
        var er = this.resolveError(error);
